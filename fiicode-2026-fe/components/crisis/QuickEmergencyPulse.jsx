...
const QuickEmergencyPulse = ({ visible, onClose, refetch }) => {
  ...
  const handleQuickPulse = async (type) => {
    ...
    try {
      await api.post('/pulse', pulseData);
          ...
          <View className="gap-2">
            {EMERGENCY_TYPES.map((type) => (
              <Pressable
                ...
                className="flex-row items-center gap-3 rounded-2xl border border-[#333] bg-[#1A1A1A] p-4 active:bg-red-900/30">
                <Ionicons name={type.icon} size={24} color="#F87171" />
                <Text className="text-base font-semibold text-white">{type.label}</Text>
              </Pressable>
            ))}
          </View>
        </View>
