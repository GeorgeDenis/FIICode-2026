...
const UploadLostAnimalModal = ({ onComplete, onClose }) => {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const result = await PetService.uploadPetPulse(image, 'LOST');
      alert('Animal reported successfully! The AI has identified the features.');
      onComplete(result);
    } catch (error) {
      ...
      <View className="w-full max-w-md overflow-hidden rounded-3xl bg-background p-6 shadow-2xl">
        ...
        <View className="mt-4 flex-row items-start rounded-xl bg-blue-50 p-3">
          ...
          <Text className="ml-2 flex-1 text-xs text-blue-700">
            We use Artificial Intelligence to automatically extract species, color, and other
            distinguishing features to help find your pet.
          </Text>
        </View>
      </View>
    </View>
...
export default UploadLostAnimalModal;
